...
class StockService:
    def __init__(self, settings: Settings, session: Session) -> None:
        self.open_close_stock_repository: OpenCloseStockRepository = (
            OpenCloseStockRepository(settings=settings)
        )
        self.marketwatch_repository: MarketWatchRepository = MarketWatchRepository(
            settings=settings
        )
    ...
    async def get_stock_by_symbol(self, stock_symbol: str, date: date) -> StockData:
        daily_open_close_data: DailyOpenCloseStock = (
            await self.open_close_stock_repository.get_daily_open_close_sotck(
                stock_symbol, date
            )
        )

        stock_values: StockValuesData = StockValuesData.model_validate(
            daily_open_close_data.model_dump()
        )
        performance_data: PerformanceData = (
            await self.marketwatch_repository.get_stock_performance_by_symbol(
                stock_symbol
            )
        )
        competitors: list[
            CompetitorData
        ] = await self.marketwatch_repository.get_stock_competitors_by_symbol(
            stock_symbol=stock_symbol
        )

        company_name: str = (
            await self.marketwatch_repository.get_company_name_by_symbol(
                stock_symbol=stock_symbol
            )
        )
